	...
	for (let x = 0; x < boardSize; x++) {
		...
		for (let y = 0; y < boardSize; y++) {
			...
			const tile = {
				...
				x,
				y,
				// check the cordinate of tile position and mine postion are match?
...
// Base numberOfMines, retrun the random mine location
function getMinePositions(boardSize, numberOfMines) {
	...
	while (positions.length < numberOfMines) {
		...
		// check does postion already exists, if not duplicate, push to array
...
function nearbyTiles(board, { x, y }) {
	...
	for (let xOffest = -1; xOffest <= 1; xOffest++) {
		for (let yOffset = -1; yOffset <= 1; yOffset++) {
			...
			const tile = board[x + xOffest]?.[y + yOffset]
